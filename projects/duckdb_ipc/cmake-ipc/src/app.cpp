...
int main(int argc, char *argv[]) {
  int64_t in_size = 0;
  constexpr size_t buffer_size = 8096;
  uint8_t data[buffer_size];

  freopen(NULL, "rb", stdin);
  in_size = fread(data, 1, buffer_size, stdin);
  fmt::println("in_size: {}", in_size);
  ...
  if (result != 0) {
    ...
    fmt::println(stderr, "library error: {}", my_library_last_error());
  }
